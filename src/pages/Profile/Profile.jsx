import React from "react";

import Navbar from "../../components/navbar"; // เรียกใช้ path ที่ถูกต้อง
import Sidebar from "../../components/Sidebar"; // เรียกใช้ path ที่ถูกต้อง
import UserProfile from "./UserProfile"; // เรียกใช้ path ที่ถูกต้อง

export default function Profile() {
  return (
    <div>
      {/* Navbar - อยู่หน้าสุดโดยใช้ fixed position */}
      <div className="h-[68px] fixed top-0 left-0 w-full bg-white shadow-md">
        <Navbar />
      </div>

      {/* คอนเทนต์หลัก ขยับลงมาเพื่อไม่ให้ทับ Navbar */}
      <div className="flex px-12 pt-20 gap-14">  
        {/* Sidebar - ใช้ sticky เพื่อให้อยู่ข้างหน้า */}
        <div className="hidden lg:block w-72 lg:sticky lg:top-[88px] lg:self-start">
          <Sidebar />
        </div>

        {/* UserProfile - อยู่หลังสุดโดยลำดับโครงสร้าง */}
        <div className="flex-1 pb-10">
          <UserProfile />
        </div>
      </div>
    </div>
  );
}
